...
Angenommen, es gilt:  
$$
\lim\limits_{t \to T^+} \| y_{t_0, y_0}(t) \|_{\mathbb{R}^d} < \infty.
$$ Wähle eine Folge $(t_n)_n$ mit $t_n \to T^+$.  
...
##### Behauptung:  
Dann gilt  $$
v(t) \leq \alpha(t) + \int_{t_0}^{t} \alpha(s) \beta(s) e^{\int_{s}^{t} \beta(r) dr} ds,  
\quad \forall t \in [t_0, T].
$$ Speziell für $\alpha = M \in \mathbb{R}$ gilt:  $v(t) \leq M e^{\int_{t_0}^{t} \beta(r) dr}.$  
...
V(t) = B t + L \int_0^t v(s) ds.
$$  Dann gilt: $V'(t) = B + L v(t).$ Da $v(t)$ durch die ursprüngliche Annahme beschränkt ist, erhalten wir:$$
V'(t) \leq B + L (B + B t + L \int_0^t v(s) ds).
$$  Dies kann umgeschrieben werden als: $V'(t) - L V(t) \leq B + B L.$ Nun multiplizieren wir beide Seiten mit $e^{-Lt}$:$$
e^{-Lt} (V'(t) - L V(t)) \leq e^{-Lt} (B + B L).
$$  Dies ist genau die Ableitung: $(e^{-Lt} V(t))' \leq e^{-Lt} (B + B L).$ Integrieren über das Intervall $[0,t]$ ergibt:
$$
e^{-Lt} V(t) - V(0) \leq \int_0^t e^{-Ls} (B + B L) ds.
$$  Durch Umstellen folgt: $V(t) \leq (e^{Lt} - 1) \frac{B + B L}{L}$.  Da wir $V(t)$ als obere Schranke für $v(t)$ gewählt haben, folgt schließlich:
$$
